...
- 复盘校准：总进球判断保持独立；若主场弱势方近 5 场仍有稳定进球能力，模型会在不改变总进球均值的前提下，提高 1-2 / 1-3 这类“一球贡献”比分权重，减少过度偏向 0-2 / 0-3。
- 赛后修正：若大小球盘口偏小、让球盘口不支持热门穿盘，或高湿/降水环境提高闷局风险，模型会降低 3+ 进球扩张，增加 1-0 / 0-1 / 0-0 等低比分路径的权重。
...
每场比赛还会生成一个“实况上下文”层，用来修正最终判断：

- 近 5 场实际情况：优先读取 ESPN 球队赛程，展示最近比分、胜平负、场均进失球和近况分。
- 球员状态与伤病：读取 ESPN 球员榜单和伤病接口；若接口无明确伤病，只表示公开源未列出，仍需赛前核验首发。
- 天气原因：通过 Open-Meteo 按比赛城市读取小时级温度、降水、风速、湿度，并判断室内/露天影响。
- 地理位置：按球队所属国家/地区到比赛城市的大致距离、气候适应和旅行差异做小幅修正。
- 古法文化校验：使用梅花易数取数、比赛日时五行、天气五行、球队地域/队名五行和方位生克做低权重提示，权重不超过 4%，不会覆盖赔率、近况、伤病、天气等可验证信息。
